  ...
  return (
    <form
      ...
      onSubmit={async (event: SyntheticEvent<HTMLFormElement>) => {
        ...
        try {
          setSubmitting(true);
          await fetch('https://api-dev.teraphone.app/v1/public/email-signup', {
...
const ButtonLinkCTA = (props: {
  primaryText: string;
  buttonText: string;
  href: string;
  target: '_blank' | '_self' | '_parent' | '_top';
}) => {
  const { primaryText, buttonText, href, target } = props;
  return (
    ...
    <>
      ...
      >
        <Grid item xs={12} sm={6} alignSelf={{ xs: 'center', md: 'center' }}>
          <Box my={4}>
            <Typography component="h1" variant="h1" sx={{ fontSize: 56 }}>
              Voice rooms
              <br />
              for Microsoft Teams
            </Typography>
          ...
          <Box my={4}>
            {/* <EarlyAccessCTA /> */}
            <ButtonLinkCTA
              primaryText="Try Teraphone Free for 30 Days"
              buttonText="Get Started!"
              href="https://web.teraphone.app"
              target="_blank"
            />
          </Box>
        </Grid>
